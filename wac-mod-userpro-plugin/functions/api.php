...
/*daw - wac begin 01 */		
		add_action( 'pre_user_query', 'wac_query_where' );
		$query = new WP_User_Query ($query);
		remove_action( 'pre_user_query', 'wac_query_where' );
/*daw - wac end 01 */			

/*daw - wac begin 01.1 this is the code that existed prior to me adding the bit above				
...
//daw - wac begin 02. Set display_name to firstname_lastname_user_id (a tiebreaker)
//and set user_nicename to username_firstname_lastname truncated to 50 characters.		
//may need to do this with social media, e.g. facebook, login if we turn that on.
		$user = get_userdata($user_id);
		$dispname = trim(substr($user->first_name . " " .$user->last_name,0,50));
		//Nicename needs to be unique - by definition it will if we put the user_login at the start of it.
		$nicename = sanitize_title(str_replace(" ","_",trim(substr($user_login . " " . $user->first_name . " " .$user->last_name,0,50))));
		if ($type == 'standard'){
			$this->set_default_display_name($user_id, $dispname);
		}

...
/*
 * daw - wac begin 01. Function referred to by add_action in memberlist-functions.php to enable memberlist by group.  Adds "where" conditions to query. 
 * note: this entire function is custom and does not exist in any form in the original code.
 */
	function wac_query_where( $query ) {
		log_me ('wac_query_where 1:'.$query->query_where);
		global $wpdb;
		$queryarr = $query->query_vars;
		//Add to the WHERE conditions if there are any in_group arguments provided
		if (isset($queryarr['in_group'])){
			$groups = explode(',',esc_attr(trim($queryarr['in_group']))); 	//get the group string from the query object and make it an array
			$subqor = '';
			$subqand = '';
			
			//Figure out if one of the arguments is AND, and if it is remove from the array and create the AND sql clause
			if(count($groups) != count(array_diff($groups, array("AND", "and")))){
				$groups = array_merge(array_diff($groups, array("AND", "and")));
				$i = count($groups);
				$subqand = ' GROUP BY a.user_id HAVING (count(a.user_id) >= '. $i .')';	//effectively changes the subquery from OR to AND
			} else {
				$i = count($groups);										
			}
			
			//Create the OR part of the subquery if there are more than one group arguments.			
			foreach($groups as $group){
				$subqor .= 'b.name =  "' . esc_attr(trim($group)) . '" ';   //add each item to the subquery condition, e.g. b.name = "a" ...etc
				if ($i-- > 1) $subqor .= ' OR ';
			}

			//Add these additional WHERE conditions to the user query
			$user_group_table = _groups_get_tablename( 'user_group' );    	//using function from Groups plugin to get group user table name.
			$group_table = _groups_get_tablename( 'group' );              	//using function from Groups plugin to get group table name.
			$user_table_ID = trim($wpdb->prefix . 'users.ID');
			$query->query_where .= ' AND ' . $user_table_ID . ' IN (SELECT a.user_id FROM ' . $user_group_table . ' AS a INNER JOIN ' . $group_table . ' AS b ON a.group_id = b.group_id WHERE (' . $subqor . ' )' . $subqand . ')';
			log_me ('wac_query_where 2:'.$query->query_where);
		}
}
//daw - wac end 01
...
// daw - wac begin 02.  Allows parsing of ingroup parameter from shortcode.
		$ingroup = (isset($args['ingroup'])) ? $args['ingroup'] : '';
// daw - wac end 02
		$arr=array();
...
//daw - wac begin 03. Catch the group parameter passed in on the shortcode.  Usage ingroup="Active Members"
		if (isset($ingroup)){
			$query['in_group'] = $ingroup;  //and store it in the query array - so action handler and find it and so userpro caching will see it.
		}
		log_me ('userpro_memberlist_loop: ingroup '.$ingroup);
//daw - wac end 03.	
	...
	/* Start Added By Yogesh for sorting the users*/
		...
		/* Search mode */
//daw - wac begin/end 05.  Always || true in conditional so group counts work accurately
		if ( ( isset($_GET['searchuser']) && !empty($_GET['searchuser']) ) || count($query['meta_query']) > 1 || true){
		...
		if ($per_page) {
			...
			/* Get Total Users */
//daw - wac begin/end 06.  Always || true in conditional so group counts work accurately
			if ( ( isset($_GET['searchuser']) && !empty($_GET['searchuser']) ) || count($query['meta_query']) > 1 || true){
